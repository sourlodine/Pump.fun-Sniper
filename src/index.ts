  ...
  try {
    ...
    globalLogListener = logConnection.onLogs(
      ...
      async ({ logs, err, signature }) => {
        ...
        if (isMint && !isBuying) {
          const parsedTransaction = await logConnection.getParsedTransaction(
            signature,
          ...
          const mint =
            parsedTransaction?.transaction.message.accountKeys[1].pubkey;
          if (isDevMode) {
          ...
          if (isTickerMode) {
            if (!tokenTicker)
              return console.log("Token Ticker is not defiend!");
          ...
          if (!sig) {
            ...
            console.log(
              "Try to sell on pumpfun: ",
              `https://pump.fun/${mint.toString()}`
            );
          }
        }
      },
